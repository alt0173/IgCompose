...
namespace fpf_scan {
	...
	public:
		...
		inline fion_node_type& nt_return_fion_head_ptr() {
			// nt_return_fion_head_ptr() returns nt_fion_head_ptr, a pointer to
			// the first fpf_fion::fion class in a doubly linked list as defined
			// in fpf_node. Collectively the nodes of the list represent the
			// fragment ions of a associated to a precursor ion.
			return nt_fion_head_ptr;
		};

		inline fion_node_type& nt_return_fion_tail_ptr() {
			// nt_return_fion_tail_ptr() returns nt_fion_tail_ptr, a pointer to
			// the last fpf_fion::fion class in a doubly linked list as defined
			// in fpf_node. Collectively the nodes of the list represent the
			// fragment ions associated to a precursor ion.
			return nt_fion_tail_ptr;
		};

		inline size_type return_scan_union_count() {
			// return_scan_union_count() returns st_scan_union_count, the
			// quantity of scan classes that have been combined into the
			// referenced scan class. An unsummed scan class has a value of 1, a
			// scan class having undergone one iteration of summing has a value
			// 2, and onwards.
	...
	private:
		...
		// vt_pion_mass is a value_type that stores the value of the precursor
		// ion mass for the referenced scan class.

		value_type vt_pion_rt;
		// vt_pion_rt is a value_type that stores the value of the precursor ion
		// retention time for the referenced scan class.

		fion_node_type nt_fion_head_ptr;
		// nt_fion_head_ptr is an fion_node_type that stores a pointer to the
		...
		// nt_fion_tail_ptr is an fion_node_type that stores a pointer to the
		// last fpf_fion::fion class in a doubly linked list as defined in
		// fpf_node. Collectively the nodes of the list represent the fragment
		// ions of a scan.

		size_type st_scan_union_count;
		// st_scan_union_count is a size_type that stores the quantity of scan
		// classes that have been combined to create the referenced scan class.
	...
	void set_CONDITION_PION_MASS(value_type vt_set_CONDITION_PION_MASS) {
		// set_CONDITION_PION_MASS() assigns a value_type to
		// vt_CONDITION_PION_MASS.
	...
	inline bool_type union_pion_mass(const scan* scan_1, const scan* scan_2) {
		// union_pion_mass() is a bool_type return function that returns true if
		// two compared scan classes have vt_pion_mass values within a range of
		// vt_CONDITION_PION_MASS. A return of true is indicative that two
		// precursor ion mass-to-charge ratios are suitable for spectral
		// summing.
	...
	inline bool_type union_pion_rt(const scan* scan_1, const scan* scan_2) {
		// union_pion_mass() is a bool_type return function that returns true if
		// two compared scan classes have vt_pion_rt values within a range of
		// vt_CONDITION_PION_RT. A return of true is indicative that two
		// precursor ion retention times are suitable for spectral summing.
	...
	inline bool_type union_fion_sup_intensities(scan* scan_1, scan* scan_2) {
		// union_fion_sup_intensities() is a bool_type return function that
		// returns true if two compared scan classes have
		// fpf_fion::st_CONDITION_FION_SUP of the
		// fpf_fion::st_CONDITION_COUNT_FION_SUP most intense fragment ions
		// within a mass-to-charge ratio range of
		// fpf_fion::vt_CONDITION_FION_MZ. Initially, the
		// st_CONDITION_COUNT_FION_SUP most intense ions for compared scan
		// classes are stored in scan_1_fion_sup[] and scan_2_fion_sup[]
		...
		for (size_t i = 0; i < fpf_fion::st_CONDITION_COUNT_FION_SUP; ++i) {
			// Determines the fpf_fion::st_CONDITION_COUNT_FION_SUP most intense
			// fragment ions in scan_1 and stores in scan_1_fion_sup[].
		...
		for (size_t i = 0; i < fpf_fion::st_CONDITION_COUNT_FION_SUP; ++i) {
			// Determines the fpf_fion::st_CONDITION_COUNT_FION_SUP most intense
			// fragment ions in scan_2 and stores in scan_2_fion_sup[].
		...
		for (size_t i = 0; i < fpf_fion::st_CONDITION_COUNT_FION_SUP; ++i) {
			// Compares mass-to-charge ratio values in scan_1_fion_sup[] and
			// scan_2_fion_sup[]. union_fion_sup_intensities() returns true if
			// st_CONDITION_FION_SUP are within a mass-to-charge ratio range of
			// vt_CONDITION_FION_MZ as determined by union_fion_mz().
			if (fpf_scan_DEBUG_MODE == 2) {
	...
	size_type scan_fion_sum(scan* c_scan_ptr) {
		// scan_fion_sum() is a size_type return function that performs the
		// primary role of fragment ion summing. The declared iterators
		// nt_fion_ptr_itr_1 and nt_fion_ptr_itr_2 iterate through the fion
		// classes of the scan class pointed to by c_scan_ptr. Should the two
		// iterators not point to the same fion class and have compared
		// fpf_ion::vt_fion_mz, the fragment ion mass-to-charge ratio, within a
		// range of fpf_ion::vt_CONDITION_FION_MZ, the fragment ions are
		// considered suitable for spectral summing. Through spectral summing,
		// the mean mass-to-charge ratio is determined by
		// fpf_ion::vt_return_mean_fion_mz(), the summed intensity by
		// fpf_ion::st_return_sum_fion_intensity(), and the cumulative count of
		// how many fion classes the new fion class is composed of by
		// fpf_ion::st_return_sum_fion_union_count(). These values are stored in
		// a new fion class which is inserted subsequent to the second compared
		// fion class, nt_fion_ptr_itr_2. Following, the compared fion classes
		// are removed from memory. This process continues until all possible
		// fragment ion summations have been completed.
		...
		for (fion_node_type nt_fion_ptr_itr_1 = c_scan_ptr->nt_return_fion_head_ptr(); nt_fion_ptr_itr_1 != NULL; nt_fion_ptr_itr_1 = nt_fion_ptr_itr_1->nt_return_up_node()) {
			// nt_fion_ptr_itr_1 iterates through the fion classes of the scan
			// class pointed to by c_scan_ptr. It corresponds to the first
			// fragment ion being compared.
			for (fion_node_type nt_fion_ptr_itr_2 = c_scan_ptr->nt_return_fion_head_ptr(); nt_fion_ptr_itr_2 != NULL; nt_fion_ptr_itr_2 = nt_fion_ptr_itr_2->nt_return_up_node()) {
				// nt_fion_ptr_itr_2 iterates through the fion classes of the
				// scan class pointed to by c_scan_ptr. It corresponds to the
				// second fragment ion being compared.
				if ((nt_fion_ptr_itr_1 != nt_fion_ptr_itr_2) 
					...
					// Creates a new fion class with values determined by
					// vt_return_mean_fion_mz(), st_return_sum_fion_intensity(),
					// and st_return_sum_fion_union_count().
					...
					// Inserts new fion class after nt_fion_ptr_itr_2, the
					// second compared fragment ion.
					++count_scan_fion_sum;
	...
	inline value_type vt_return_mean_pion_mass(scan* scan_1, scan* scan_2) {
		// vt_return_mean_pion_mass() is a value_type return function that
		// returns the mean mass of two scan classes. This function is called
		// when assigning value to a combined scan class from two scan classes
	...
	inline value_type vt_return_mean_pion_rt(scan* scan_1, scan* scan_2) {
		// vt_return_mean_pion_rt() is a value_type return function that returns
		// the mean retention time of two scan classes. This function is called
		// when assigning value to a combined scan class from two scan classes
		// suitable for spectral summing. Previous summing is taken into
		// consideration through st_scan_union_count.
	...
	inline size_type st_return_sum_scan_union_count(scan* scan_1, scan* scan_2) {
		...
		// returns the sum of the union count of two scan classes. This function
		// is called when assigning value to a combined scan class from two scan
		// classes suitable for spectral summing. The cumulative value of
		// st_scan_union_count represents the total number of scan classes that
		// have been combined into the new combined scan class.
	...
	inline void set_scan_fion_union(scan*& scan_union, scan* scan_1, scan* scan_2) {
		// set_scan_fion_union() is a void type function that concaternates the
		// doubly linked lists of two fion classes pointed to by two scan
		// classes determined to be suitable for spectral summing. The list from
		// the first scan class is assigned to a new list and the list from the
		// second scan class is concaternated onto this list. Following, the new
		// doubly linked list is summed through a call to scan_fion_sum().
