...
namespace fpf_node {
	...
	public:
		...
		node(const data_type& d_init_data = data_type(), node_type nt_init_up_node = NULL, node_type nt_init_down_node = NULL) {
			// node() defines a constructor call for a node class that assigns declared parameters as
			// initial values or calls default values as ISO type defaults (0 for standard C++ data types).
			d_data = d_init_data;
	...
	void list_insert_head(const typename node<element>::data_type dt_new_data, typename node<element>::node_type& nt_node_head_ptr, typename node<element>::node_type& nt_node_tail_ptr) {
		...
		// called doubly linked list. If the called list is empty the created
