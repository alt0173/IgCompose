...
// * * * * *
 ...
 // 
 // The fpf_fion function union_fion_mz() boolean tests if two fion class
 // fragment ion mass-to-charge ratios are suitable for spectral summing - A
 // true boolean value calls vt_return_mean_fion_mz() and
 // st_return_sum_fion_intensity() to calculate the mean mass-to-charge ratio
 // and summed intensity of the two fion classes, taking into consideration
 // previous summing through st_fion_union_count. Fragment ion spectral summing
 // is called from the parent fpf_scan scan class function scan_fion_sum().
...
namespace fpf_fion {
	...
	// vt_CONDITION_FION_MZ is a value_type with a compile-time or run-time
	// initialised value that defines the fragment ion mass-to-charge ratio
	// range for spectral summing.
	...
	// vt_CONDITION_FION_MZ the two scan classes are considered to have the
	// required fragment ion distribution for spectral summing.

	class fion {
	public:

		fion() {
			// fion() defines the default constructor for the fpf_fion::fion
			// class that initialises member values as ISO type defaults (0 for
			// standard C++ data types) and the value 1 for st_scan_union_count
		...
		~fion() {
			// ~fion() defines the default destructor for the fpf_fion::fion class.
		};
		...
		inline value_type vt_return_fion_mz() const {
			// vt_return_fion_mz() returns vt_fion_mz, the fragment fion
			// mass-to-charge ratio of the referenced fion class.
			return vt_fion_mz;
		};

		inline size_type return_fion_intensity() const {
			// vt_return_fion_mz() returns st_fion_intensity, the fragment fion
			// detected intensity of the referenced fion class.
		...
		inline size_type st_return_union_count() const {
			...
			// fragment ion spectra that have been combined into the referenced
			// fion class.
	...
	private:
		...
		// vt_fion_mz is a value_type data type that stores the value of a
		// fragment ion mass-to-charge ratio in the referenced fion class.

		size_type st_fion_intensity;
		// st_fion_intensity is a size_type data type that stores the value of a
		// fragment ion detected intensity in the referenced fion class.

		size_type st_fion_union_count;
		// st_fion_union_count is a size_type data type that stores the quantity
		// of fion classes that have been combined to create the referenced fion
	...
	inline bool_type union_fion_mz(const data_type& d_fion_itr_1, const data_type& d_fion_itr_2) {
		...
		// fragment fion mass-to-charge ratios are suitable for spectral
		// summing.
	...
	inline value_type vt_return_mean_fion_mz(fion* fion_1, fion* fion_2) {
		// vt_return_mean_fion_mz() is a value_type return function that returns
		// the mean mass-to-charge ratio of two fion classes. This function is
		// called when assigning value to a combined fion class from two fion
		// classes suitable for spectral summing. Previous summing is taken into
	...
	inline size_type st_return_sum_fion_intensity(fion* fion_1, fion* fion_2) {
		// st_return_sum_fion_intensity() is a size_type return function that
		// returns the sum of the intensity of two fion classes. This function
		// is called when assigning value to a combined fion class from two fion
		// classes suitable for spectral summing. Previous summing is taken into
		// consideration through st_fion_union_count.
	...
	inline size_type st_return_sum_fion_union_count(fion* fion_1, fion* fion_2) {
		...
		// returns the sum of the union count of two fion classes. This function
		// is called when assigning value to a combined fion class from two fion
		// classes suitable for spectral summing. The cumulative value of
		// st_fion_union_count represents the total number of fion classes that
		// have been combined into the new combined fion class.
